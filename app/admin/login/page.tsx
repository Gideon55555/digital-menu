      ...
      <div className="min-h-screen bg-gradient-to-br from-cream-100 to-white dark:from-restaurant-bg-dark dark:to-slate-900 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-500">
            {isAmharic ? 'አካውንት በመፈተሽ ላይ...' : 'Checking session...'}
          </p>
        </div>
      </div>
    );
        ...
        <div className="w-full max-w-md">
          <div className="restaurant-card p-8 text-center space-y-6">
            ...
            <div className="flex justify-end">
              <div className="flex items-center rounded-lg border border-cream-200 dark:border-slate-800 bg-cream-50 dark:bg-slate-800 p-0.5 shadow-sm text-xs">
                <button
                  type="button"
                  onClick={() => handleLanguageChange('en')}
                  className={`px-2.5 py-1 rounded-md font-bold transition-all ${
                    !isAmharic ? 'bg-restaurant-accent text-white' : 'text-gray-600 dark:text-gray-300'
                  }`}
                >
                  EN
                </button>
            ...
            <div>
              <h1 className="text-2xl font-serif font-bold text-restaurant-text dark:text-white">
                {isAmharic ? 'ንቁ አካውንት ተገኝቷል' : 'Active Session Detected'}
              </h1>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {isAmharic ? 'በአሁኑ ሰዓት የገቡበት አካውንት:' : 'You are currently signed in as:'}
              </p>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
              {isAmharic
                ? 'የሲስተሙን ደህንነት ለመጠበቅ እና የስራ ሚና እንዳይቀላቀል፣ በዚህ አካውንት እንደገቡ ይቆያሉ። በሌላ አካውንት ለመግባት መጀመሪያ መውጣት አለብዎት።'
            ...
            <div className="flex flex-col gap-2.5 pt-2">
              ...
              >
                {isAmharic ? 'ወደ ዳሽቦርድ ቀጥል' : `Continue to ${roleName.replace('_', ' ')} Dashboard`}
                <ArrowRight size={16} />
              </button>
      ...
      <div className="w-full max-w-md">
        <div className="restaurant-card p-8">
          ...
          <p className="text-center text-restaurant-text-light dark:text-gray-400 mb-8 text-sm">
            {isAmharic ? 'የሬስቶራንት አስተዳደር ፖርታል' : 'Restaurant Management Portal'}
          </p>
          ...
          <div className="mt-8 p-4 bg-cream-50 dark:bg-slate-800 rounded-lg">
            <p className="text-xs text-restaurant-text-light dark:text-gray-400 text-center">
              {isAmharic
                ? 'በተሰጠዎት የሰራተኛ ወይም የአስተዳዳሪ መለያ ይግቡ።'
